...
import Image     #import python image library
from naoqi import *  
...
def showNaoImage(IP, PORT,camID):# 参数分别为IP、PORT、摄像头ID（区分上下摄像头）
    ...
    frameArray = numpy.frombuffer(naoImage[6], dtype=numpy.uint8).reshape(imageHeight, imageWidth, imagechannls)
    ...
    cnts = cv2.FindContours(thresh,storage,cv2.RETR_LIST,cv2.CHAIN_APPROX_SIMPLE)
    currtnt=cnts
    Area = 0
    left_right = 0
    up_down = 0
           ...
           if area > areamin:
             if area < areamax:
                if radio > 0.6:
                    if radio < 1.6:
                      # if radio_c == 1:
                       cv2.DrawContours(frameArray, cnts, (255, 255, 0), (255, 255, 0), 0, 1)#画出单连通轮廓
                       cv2.Rectangle(frameArray,(rect[0],rect[1]),(rect[0]+rect[2],rect[1]+rect[3]),(0,0,255),1)#画出矩形框
