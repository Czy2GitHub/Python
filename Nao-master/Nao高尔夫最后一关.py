...
# polates 插值 插入内容 stiffness 刚度
def StiffnessOn(proxy):     # 刚化设置
    # 设置全身关节刚化
...
def kaiBall():
    ...
    time.sleep(0)
    motionPrx.angleInterpolationWithSpeed(PositionJointNamesR, golfPositionJointAnglesR101, 0.5)
    time.sleep(1)
...
# 黄杆有关 ------------------------------------------------#
def findYellowStick(contourRange = [150, 300]):
    ...
    frameBin = cv2.inRange(frameHSV, low, up)           # 返回一个二值图，
...
def findball(angle, sub_angle=0):
    ...
    # 测试用
    if (ballData != []):
        tts.say("The red ball is on" + str(angle) + "angle")
        # 获取当前头的角度
    ...
    # 测试修改的
        ...
        return []
    # golbal 为声明一个局部变量，这个变量与全局的其他同名变量无任何关系
...
def CalculateRobotToRedball(allballData):
    ...
    refixRound(theta1)
    tts.post.say("I can see The Ball!")
    # 走到三十厘米左右
    val1 = findball(0)
    if val1 != []:
        # 顺便修正一下角度
...
def AdjustPosition(turndata):
    ...
    motionPrx.angleInterpolationWithSpeed("HeadPitch", 0.2, 0.3)
